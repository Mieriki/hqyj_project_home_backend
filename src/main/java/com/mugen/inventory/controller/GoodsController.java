...
public class GoodsController {
    ...
    @PostMapping("/get")
    public <T>RestBean<GoodsPageVo> queryPage(@RequestBody @Validated GoodsQueryPageVo vo) {
        return RestBean.success(service.queryPage(vo));
    }

    @PostMapping("/get/store")
    public <T>RestBean<GoodsPageVo> queryStorePage(@RequestBody @Validated GoodsQueryPageVo vo) {
        return RestBean.success(service.queryStorePage(vo));
    }

    @PostMapping("/get/cart")
    public <T>RestBean<GoodsPageVo> queryInstallPage(@RequestBody @Validated GoodsQueryPageVo vo) {
        return RestBean.success(service.queryInstallPage(vo));
    }

    @GetMapping("/get/{id}")
    public <T>RestBean<Goods> query(@PathVariable Integer id) {
        return RestBean.success(service.getById(id));
    }

    @PostMapping("/post")
    public <T>RestBean<Void> save(@RequestBody @Validated Goods vo) {
        return RestBean.messageHandle(vo, service::saveHandler);
    }

    @PostMapping("/put")
    public <T>RestBean<Void> modify(@RequestBody @Validated Goods vo) {
    ...
    @GetMapping("/get/card")
    public <T>RestBean<List<Goods>> queryCard() {
        return RestBean.success(service.queryCard());
    }

    @SneakyThrows
    @GetMapping("/get/excel")
    public void exportData(HttpServletResponse response) {
        String fileName = "Goods_" + new Date() + ".xlsx";
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=" + URLEncoder.encode(fileName, "UTF-8"));
        OutputStream out = response.getOutputStream();
        List<Goods> rowss = CollUtil.newArrayList();
    ...
    public <T> RestBean<Void> handleFileUpload(HttpServletRequest request, @RequestParam("file") MultipartFile file) {
        ...
        return RestBean.messageHandle(goodsList, service::saveHandler);
    }
}
