 ...
 */
@RestController
@RequestMapping("/purchaseListGoodss")
public class PurchaseListGoodsController {
    @Resource
    private PurchaseListGoodsService service;
    ...
    @GetMapping("/get/{id}")
    public <T>RestBean<PurchaseListGoods> query(@PathVariable Integer id) {
        return RestBean.success(service.getById(id));
    }

    @PostMapping("/post")
    public <T>RestBean<Void> save(@RequestBody @Validated PurchaseListGoods vo) {
        return RestBean.messageHandle(vo, service::saveHandler);
    }

    @PostMapping("/put")
    public <T>RestBean<Void> modify(@RequestBody @Validated PurchaseListGoods vo) {
    ...
    public void exportData(HttpServletResponse response) {
        String fileName = "PurchaseListGoods_" + new Date() + ".xlsx";
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=" + URLEncoder.encode(fileName, "UTF-8"));
        OutputStream out = response.getOutputStream();
        List<PurchaseListGoods> rowss = CollUtil.newArrayList();
    ...
    public <T> RestBean<Void> handleFileUpload(HttpServletRequest request, @RequestParam("file") MultipartFile file) {
        ...
        List<PurchaseListGoods> purchaselistgoodsList = reader.readAll(PurchaseListGoods.class);
        return RestBean.messageHandle(purchaselistgoodsList, service::saveHandler);
    }
}
