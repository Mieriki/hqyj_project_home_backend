...
public class GoodsTypeController {
    ...
    @GetMapping("/get/{id}")
    public <T>RestBean<GoodsType> query(@PathVariable Integer id) {
        return RestBean.success(service.getById(id));
    }

    @PostMapping("/post")
    public <T>RestBean<Void> save(@RequestBody @Validated GoodsType vo) {
        return RestBean.messageHandle(vo, service::saveHandler);
    }

    @PostMapping("/put")
    public <T>RestBean<Void> modify(@RequestBody @Validated GoodsType vo) {
    ...
    @GetMapping("/get/tree")
    public <T>RestBean<List> queryTree() {
        return RestBean.success(service.queryTree());
    }

    @SneakyThrows
    @GetMapping("/get/excel")
    public void exportData(HttpServletResponse response) {
        String fileName = "GoodsType_" + new Date() + ".xlsx";
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=" + URLEncoder.encode(fileName, "UTF-8"));
        OutputStream out = response.getOutputStream();
        List<GoodsType> rowss = CollUtil.newArrayList();
    ...
    public <T> RestBean<Void> handleFileUpload(HttpServletRequest request, @RequestParam("file") MultipartFile file) {
        ...
        return RestBean.messageHandle(goodstypeList, service::saveHandler);
    }
}
